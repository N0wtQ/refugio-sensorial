...
// Presentational metadata only — nombre/desc/accion come from the 'ayuda'
// i18n namespace as parallel arrays, matched here by index.
const URGENTES_META = [
  { href: 'tel:717003717', icon: 'fa-phone', color: 'coral' },
  { href: 'tel:112', icon: 'fa-truck-medical', color: 'coral' },
]

const APOYO_META = [
  { href: 'https://autismo.org.es', icon: 'fa-infinity', color: 'pri' },
  { href: 'https://www.autismomadrid.es', icon: 'fa-map-location-dot', color: 'sec' },
  { href: 'https://www.fundacionconectea.org/', icon: 'fa-people-group', color: 'acc' },
  { href: 'https://stopsuicidios.org', icon: 'fa-heart', color: 'sec' },
]
...
const PHASES_META = [
  { duration: 4, color: '#3A82CA' },
  { duration: 7, color: '#816AB7' },
  { duration: 8, color: '#48B0A1' },
]

function BreathingExercise({ prefersReduced }) {
  const { t } = useTranslation('ayuda')
  const PHASES = PHASES_META.map((p, i) => ({
    ...p,
    label: [t('tecnicas.breathing.phaseInhala'), t('tecnicas.breathing.phaseManten'), t('tecnicas.breathing.phaseExhala')][i],
  }))
  ...
  return (
    <div className="flex flex-col items-center gap-5 p-6 rounded-card bg-surface border border-border">
      <div className="flex items-center gap-3 w-full">
        ...
        <h3 className="font-semibold text-text text-sm flex-1">{t('tecnicas.breathing.titulo')}</h3>
        <TTSButton text={t('tecnicas.breathing.tts')} iconOnly />
      </div>
      ...
      <div className="relative w-32 h-32">
        ...
        >
          {running ? (
            ...
            <span className="text-xs text-faint text-center px-2">{t('tecnicas.breathing.pulsaLine1')}<br/>{t('tecnicas.breathing.pulsaLine2')}</span>
          )}
        </div>
      </div>
      ...
      <p className="text-xs text-acc font-medium min-h-[1.1rem]" aria-live="polite" aria-atomic="true">
        {cycles > 0 ? t('tecnicas.breathing.cycles', { count: cycles }) : ''}
      </p>

      <div className="flex gap-2">
        <button
          onClick={() => running ? stop() : setRunning(true)}
          aria-label={running ? t('tecnicas.breathing.stopAriaLabel') : t('tecnicas.breathing.startAriaLabel')}
        ...
        >
          {running ? t('tecnicas.breathing.detener') : t('tecnicas.breathing.comenzar')}
        </button>
      </div>
    </div>
  )
...
// ── 5-4-3-2-1 Grounding ───────────────────────────────────────────────────
const GROUNDING_5_META = [
  { n: 5, senseKey: 'senseVer',      icon: 'fa-eye',        color: '#3A82CA' },
  { n: 4, senseKey: 'senseTocar',    icon: 'fa-hand',       color: '#816AB7' },
  { n: 3, senseKey: 'senseOir',      icon: 'fa-ear-listen', color: '#48B0A1' },
  { n: 2, senseKey: 'senseOler',     icon: 'fa-wind',       color: '#9CC156' },
  { n: 1, senseKey: 'senseSaborear', icon: 'fa-droplet',    color: '#FBB027' },
]

function GroundingCard() {
  const { t } = useTranslation('ayuda')
  const GROUNDING_5 = GROUNDING_5_META.map(g => ({ ...g, sense: t(`tecnicas.grounding.${g.senseKey}`) }))
  ...
  return (
    <div className="flex flex-col gap-4 p-6 rounded-card bg-surface border border-border">
      <div className="flex items-center gap-3">
        ...
        <h3 className="font-semibold text-text text-sm flex-1">{t('tecnicas.grounding.titulo')}</h3>
        <TTSButton text={t('tecnicas.grounding.tts')} iconOnly />
      </div>
        ...
        {!done ? (
          ...
          >
            ...
            <p className="text-center">
              ...
              <span className="text-xs text-muted mt-1 block">{t('tecnicas.grounding.cosasQuePuedes')} <strong className="text-text">{cur.sense}</strong> {t('tecnicas.grounding.ahoraMismo')}</span>
            </p>
          </motion.div>
        ) : (
          <motion.div
          ...
          >
            ...
            <p className="text-sm font-semibold text-text">{t('tecnicas.grounding.completado')}</p>
            <p className="text-xs text-muted">{t('tecnicas.grounding.tomateMomento')}</p>
          </motion.div>
        )}
      </AnimatePresence>
      ...
      <div className="flex gap-2">
        {!done ? (
          <button
            ...
            aria-label={
              step + 1 < GROUNDING_5.length
                ? t('tecnicas.grounding.siguienteAriaLabel', { n: GROUNDING_5[step + 1].n, sense: GROUNDING_5[step + 1].sense })
                : t('tecnicas.grounding.completarAriaLabel')
            }
          ...
          >
            {t('tecnicas.grounding.siguiente')} <span aria-hidden="true">→</span>
          </button>
        ) : (
          <button
            onClick={() => setStep(0)}
...
const COLD_STEPS_ICONS = ['fa-droplet', 'fa-hand', 'fa-snowflake', 'fa-circle-info']

function ColdWaterCard() {
  const { t } = useTranslation('ayuda')
  const steps = t('tecnicas.coldWater.steps', { returnObjects: true })
  ...
  return (
    <div className="flex flex-col gap-4 p-6 rounded-card bg-surface border border-border">
      <div className="flex items-center gap-3">
        ...
        <h3 className="font-semibold text-text text-sm flex-1">{t('tecnicas.coldWater.titulo')}</h3>
        <TTSButton text={t('tecnicas.coldWater.tts')} iconOnly />
      </div>
        ...
        {steps.map((text, i) => (
          <div key={i} className="flex items-start gap-3">
            <div className="shrink-0 w-7 h-7 rounded-lg bg-pri/8 border border-pri/15 flex items-center justify-center mt-0.5">
              <i className={`fa-solid ${COLD_STEPS_ICONS[i]} text-pri text-[11px]`} aria-hidden="true" />
            </div>
            <p className="text-xs text-muted leading-relaxed pt-1">{text}</p>
          </div>
        ))}
      </div>
      <div className="flex items-center gap-2 p-3 rounded-xl bg-pri/6 border border-pri/15">
        ...
        <p className="text-xs text-muted">{t('tecnicas.coldWater.tip')}</p>
      </div>
    </div>
  )
}
...
export default function AyudaPage() {
  ...
  const URGENTES = t('urgentes.items', { returnObjects: true }).map((item, i) => ({ ...item, ...URGENTES_META[i] }))
  const APOYO = t('apoyo.items', { returnObjects: true }).map((item, i) => ({ ...item, ...APOYO_META[i] }))
    ...
    <div className="max-w-3xl mx-auto px-4 pb-20 pt-8">
      ...
      <nav aria-label={t('breadcrumbAriaLabel')} className="mb-6 text-sm text-faint">
        <ol className="flex items-center gap-2 list-none p-0 m-0">
          ...
          <li><span className="text-muted" aria-current="page">{t('breadcrumbCurrent')}</span></li>
        </ol>
      </nav>
      ...
      >
        ...
        <h1 className="text-3xl font-bold text-text mb-3">{t('header.heading')}</h1>
      ...
      <section aria-labelledby="sonidos-h" className="mb-10">
        ...
        <div className="flex flex-col gap-4">
          ...
          <a
            ...
            aria-label={t('sonidos.lilPenguin.ariaLabel')}
          >
            {/* YouTube icon badge */}
        ...
        <div className="grid sm:grid-cols-2 gap-4">
          {APOYO.map(r => (
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              aria-label={`${r.nombre}: ${r.desc} (${t('apoyo.ariaLabelSuffix')})`}
            ...
            >
              ...
              <span className="text-xs text-faint flex items-center gap-1">
                {t('apoyo.visitarWeb')} <i className="fa-solid fa-arrow-up-right-from-square text-[9px]" aria-hidden="true" />
              </span>
            </a>
          ))}
        </div>
