...
export default function AccesoPage() {
  usePageMeta({
    title: 'Acceso — Comunidad | Refugio Sensorial',
    description: 'Inicia sesión o regístrate para participar en la comunidad de Refugio Sensorial.',
    noIndex: true,
  })
  const { user, signIn, signUp, signInWithGoogle, configured } = useAuth()
  const navigate = useNavigate()
  const location = useLocation()
  const from = location.state?.from?.pathname ?? '/comunidad'
  ...
  return (
    <div className="max-w-md mx-auto px-4 py-12">
      ...
      <p className="text-muted text-sm mb-8">
        Para compartir y gestionar tus espacios favoritos con la comunidad.
      </p>
